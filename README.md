...
同时，它也可以作为轻量的 OpenAI-compatible Chat Completions 透明 relay：客户端直接调用 `/v1/chat/completions`，由 Chat2Responses 负责鉴权、模型映射和转发。
...
- `POST /v1/chat/completions`：接收 Chat Completions 请求，按模型映射透明转发到上游。
...
| `POST /v1/chat/completions` | 接收 Chat Completions 请求，替换公开模型名为上游模型名后原样转发。 |
