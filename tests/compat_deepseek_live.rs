...
//!
//! NOTE: these live tests still document the pre-service harness shape and
//! need to be migrated to initialize upstreams/models/keys through the admin
//! API before they are useful against the managed service entrypoint.
//!
...
impl Relay {
    fn spawn(upstream: &str, key: &str) -> Self {
        ...
        let child = Command::new(RELAY_BIN)
            ...
            .env("CHAT2RESPONSES_UPSTREAM", upstream)
            .env("CHAT2RESPONSES_API_KEY", key)
...
//
// What we *can* lock in live, however, is that Chat2Responses' outbound Chat
...
// vision, Chat2Responses users get it for free without further changes.
//
